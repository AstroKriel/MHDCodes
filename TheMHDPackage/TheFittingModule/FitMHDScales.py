...
class SpectraModels():
  ...
  def tail_linear(x, a0, a1):
    ''' powerlaw (energy spectra decay) in linear-domain
      y   = 10^a0 * k^a1
    '''
    return 10**(a0) * np.array(x)**(a1)
  def tail_log10(x_log10, a0, a1):
    ''' powerlaw (energy spectra decay) in log10-domain
...
## ###############################################################
class FitSpectra(metaclass=abc.ABCMeta): # sbstract base class (ABC)
  def __init__(
      self,
    ...
    self.list_fit_std_group_t      = [] # uncertainties in paramater fits
  ...
  def fitTimeRealisation(
    ...
    ## define a cutoff k-mode
    if self.bool_fit_sub_y_range:
      ## find the k-mode where the energy spectra is closest to the cutoff energy
      k_end = int(WWLists.getIndexClosestValue(
        np.log10(np.array(data_power)),
        -(self.num_decades_to_fit)
      ))
    elif self.k_end is None:
      ## fit up to the final k-mode
      k_end = int(data_k[-1])
    ## use the defined cut-off k-mode
    else: k_end = self.k_end
    ## ###################################
    ...
    bf_list_fit_params   = list_fit_params_group_k[index_best_fit]
    bf_list_fit_std      = list_params_std_group_k[index_best_fit]
    bf_max_k_mode_fitted = list_fit_k_range[index_best_fit]
    ...
    ## save fitted scales
    a0_b, a1_b, a2_b = bf_list_fit_params # get best fit parameters
    powerlaw_slope   = a1_b
    k_scale          = 1 / a2_b
...
class FitKinSpectra(FitSpectra):
  def __init__(
    ...
    FitSpectra.__init__(
      ...
      k_start              = self.fit_k_start,
      k_break_from         = k_break_from,
  ...
  def getFitDict(self):
    ...
    return {
      ...
      "kin_fit_k_start":               self.fit_k_start,
...
class FitMagSpectra(FitSpectra):
  ...
  def getFitDict(self):
    ...
    return {
      ...
      "mag_fit_k_start":               self.fit_k_start,
